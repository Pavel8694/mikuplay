...
async def show_admin_menu_button(callback_query: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await callback_query.message.edit_text('👮 *Админка:*\n'
                                           '*➕ Добавить треки — начать добавление композиций в базу данных бота.*\n'
                                           '*✏️ Редактировать трек — изменить информацию о треке (название, исполнитель) в базе данных бота.* _(Это не меняет метаданные MP3-файла, а лишь меняет информацию о треке в базе данных бота. Актуально только для главного администратора бота, т. е. его разработчика.)_\n'
                                           '*🔁 Заменить аудио-файлы — заменить текущие файлы треков другими MP3-файлами с заменой их данных в БД (названий, исполнителей).*\n'
                                           '*❌ Удалить трек — скрыть трек из поисковой выдачи.* _(В базе данных останется запись об ID трека, его названии и исполнителе, но будет удалён его fileID, это скроет трек из поисковой выдачи бота. Актуально только для главного администратора бота, т. е. его разработчика.)_\n'
                                           '*👤 Добавить администратора — добавить администратора в бота по userID.* _(Актуально только для главного администратора бота, т. е. его разработчика.)_\n'
                                           '*🥵 Разжаловать администратора — удалить администратора из бота по userID.* _(Актуально только для главного администратора бота, т. е. его разработчика.)_\n'
                                           '*🆔 Получить ID профиля и чата — узнать ID вашего Telegram профиля и текущего чата.*\n'
                                           '*📂 Получить ID файла — узнать ID отправляемого вами файла для бота MikuPlay.* _(Актуально только для главного администратора бота, т. е. его разработчика.)_\n\n'
                                           '💙 *Выберите действие:*',
                                           parse_mode="Markdown",
