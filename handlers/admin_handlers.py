...
MAIN_ADMIN_ID = 1331018098 # Айди главного админа
...
async def show_admin_menu_button(callback_query: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await callback_query.message.edit_text('👮 *Админка.*\n\n'
                                           '💙 *Выберите действие:*',
                                           parse_mode="Markdown",
                                           reply_markup=get_amdin_menu_keyboard
                                           )
    
@admin_router.callback_query(F.data == "admin_info_menu_button")
@admin_only
async def show_admin_info_menu_button(callback_query: CallbackQuery, state: FSMContext, session: AsyncSession):
    logger.info(f"ℹ️ Пользователь @{callback_query.from_user.username} ({callback_query.from_user.id}) зашёл в подробности функционала меню администраторов.")
    await callback_query.message.edit_text('ℹ️ <b>Подробнее о действиях в админке:</b>\n'
                                           '<b>➕ Добавить треки — начать добавление композиций в базу данных бота.</b>\n'
                                           '<b><i>✏️ Редактировать трек — изменить информацию о треке (название, исполнитель) в базе данных бота.</i></b> <i>(Это не меняет метаданные MP3-файла, а лишь меняет информацию о треке в базе данных бота.)</i>\n'
                                           '<b>🔁 Заменить аудио-файлы — заменить текущие файлы треков другими MP3-файлами с заменой их данных в БД (названий, исполнителей).</b>\n'
                                           '<b><i>❌ Удалить трек — скрыть трек из поисковой выдачи.</i></b> <i>(В базе данных останется запись об ID трека, его названии и исполнителе, но будет удалён его fileID, это скроет трек из поисковой выдачи бота.)</i>\n'
                                           '<b><i>👤 Добавить администратора — добавить администратора в бота по userID.</i></b>\n'
                                           '<b><i>🥵 Разжаловать администратора — удалить администратора из бота по userID.</i></b>\n'
                                           '<b>🆔 Получить ID профиля и чата — узнать ID вашего Telegram профиля и текущего чата.</b>\n'
                                           '<b><i>📂 Получить ID файла — узнать ID отправляемого вами файла для бота MikuPlay.</i></b>\n\n'
                                           '<b>✍️(◔◡◔)</b>\n'
                                           '<b><i>Жирный шрифт + курсив — Актуально только для главного администратора бота, т. е. его разработчика.</i></b>',
                                           parse_mode="HTML",
...
async def delete_track_command(callback_query: CallbackQuery, state: FSMContext, session: AsyncSession):
    if callback_query.from_user.id != MAIN_ADMIN_ID:
        await callback_query.message.edit_text("❌ *Только главный администратор может удалять треки.*", parse_mode="Markdown", reply_markup=back_admin_menu_keyboard)
        logger.info(f"❌ Пользователь @{callback_query.from_user.username} ({callback_query.from_user.id}) пытался зайти в меню удаления треков.")
        return
